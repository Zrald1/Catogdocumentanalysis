...
// --- Hallucination & Uncertainty Patterns ---

var HallucinationPatterns = compile("hallucination_signals", []string{
	// Phrases indicating uncertainty or potential fabrication
	`(?i)\b(i\s+think|i\s+believe|it\s+is\s+possible|most\s+likely|could\s+be|might\s+be|potentially)\b`,
	`(?i)\b(i\s+am\s+not\s+(entirely\s+)?sure|i\s+cannot\s+verify|no\s+direct\s+evidence|insufficient\s+data|incomplete\s+information)\b`,
	`(?i)\b(i\s+don'?t\s+have\s+access\s+to\s+the\s+actual|i\s+am\s+speculating|this\s+is\s+a\s+guess)\b`,
	// Indicators of over-confidence in missing data
	`(?i)\b(i\s+can\s+confirm\s+that\s+none\s+of\s+the|there\s+are\s+definitely\s+no|it\s+is\s+a\s+known\s+fact\s+that)\b`,
	// Apologetic fillers often preceding hallucinations
	`(?i)\b(i\s+apologize|i\s+must\s+correct\s+myself|actually\s+i\s+find)\b`,
	// Conflict markers (stating multiple contradictory things)
	`(?i)\b(on\s+the\s+other\s+hand|however\s+it\s+could\s+also\s+be)\b`,
})
